                ...
                <div class="row">
                    <div class="col-12">
                        <div class="reviews__header">
                            <h2 class="reviews__title">{{__('Customer reviews')}}</h2>
                        </div>
                        @auth
                            @include('partials.inform')
                            <button type="button" id="open-review" class="add-review-btn add-review-btn--open-popup" href="#">
                                {{__('Post a review')}}
                            </button>
                            <div class="leave-review">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
                                <div class="leave-review__mob-title">{{__('Post a review')}}</div>
                                <div class="leave-review__title">{{__('You can leave your feedback about our work.')}}</div>
                                <form method="POST" name="leave-review__form" role="form" id="leave_review_form" action="{{ route('reviews.add') }}">
                                    {{ csrf_field() }}
                                    <input type="hidden" name="type_id" value="2">
                                    <div class="leave-review__field-container">
                                        <label for="video-link" class="leave-review__label">{{__('Video link')}}</label>
                                        <input type="text" name="video" id="video-link" class="leave-review__field" placeholder="https://youtu.be/XXXXXXXXXXX">
                                    </div>
                                    <div class="leave-review__field-container">
                                        <label for="review-text" class="leave-review__label leave-review__label--textarea">{{__('Your feedback:')}}</label>
                                        <textarea type="text" id="review-text"  name="text" class="leave-review__field" cols="90" rows="7" placeholder="{{__('Enter text')}}"></textarea>
                                    </div>
                        ...
                        @endauth
                        <div class="reviews-container reviews-container--video">
                            @guest()
