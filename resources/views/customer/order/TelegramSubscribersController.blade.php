...
@section('title', __("Subscribers for Telegram") . ' - ' . __('site.site'))
...
@section('content')
    <div class="row">
        <div class="service-content col-md-8">
            ...
            <form name="add_order" method="post" action="{{ route('add_order', ['service' => 'SERVICE_ADD_ORDER_TELEGRAM_SUBSCRIBERS']) }}">
                <input type="hidden" name="type" value="instagram-comments">
                <h1>{{ __("Subscribers for Telegram") }}</h1>
                <br>
                <div class="row">
                    <div class="label-name col-md-4">{{ __("Price / 1:") }}</div>
                    <div class="col-md-8 priceOne"><span id="price_base"><strong>{{ socialboosterPrice('SERVICE_ADD_ORDER_TELEGRAM_SUBSCRIBERS_PRICE') }}</strong></span> {{ session()->has('lang') && session('lang') == 'ru' ? '₽' : '$' }}</div>
                </div>
                <div class="row">
 ...
 <div class="text-page col-md-12"><!-- VK: Лайки ВКонтакте -->
            <ul>
                <li>
                    ...
                    * {{ __("Only for channels") }} <br />
                    * {{ __("Not real members") }}<br />
