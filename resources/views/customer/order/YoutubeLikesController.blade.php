...
@section('title', __("YouTube Likes") . ' - ' . __('site.site'))
...
@section('content')
    <div class="row">
        <div class="service-content col-md-8">
            ...
            <form name="add_order" method="post" action="{{ route('add_order', ['service' => 'SERVICE_ADD_ORDER_YOUTUBE_LIKES']) }}">
                <input type="hidden" name="type" value="youtube-likes">
                <h1>{{ __("YouTube Likes") }}</h1>
                <br>
                <div class="row">
                    <div class="label-name col-md-4">{{ __("Price / 1:") }}</div>
                    <div class="col-md-8 priceOne"><span id="price_base"><strong>{{ socialboosterPrice('SERVICE_ADD_ORDER_YOUTUBE_LIKES_PRICE') }}</strong></span> {{ session()->has('lang') && session('lang') == 'ru' ? '₽' : '$' }}</div>
                </div>
                {{--<div class="row">--}}
