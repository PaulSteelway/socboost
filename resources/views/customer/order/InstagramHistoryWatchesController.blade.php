...
@section('title', __("Instagram story views") . ' - ' . __('site.site'))
...
@section('content')
    <div class="row">
        <div class="service-content col-md-8">
            ...
            <form name="add_order" method="post" action="{{ route('add_order', ['service' => 'SERVICE_ADD_ORDER_INSTAGRAM_HISTORY_WATCHES']) }}">
                <input type="hidden" name="type" value="instagram-comments">
                <h1>{{ __("Instagram story views") }}</h1>
                <br>
                <div class="row">
                    <div class="label-name col-md-4">{{ __("Price / 1:") }}</div>
                    <div class="col-md-8 priceOne"><span id="price_base"><strong>{{ socialboosterPrice('SERVICE_ADD_ORDER_INSTAGRAM_HISTORY_WATCHES_PRICE') }}</strong></span> {{ session()->has('lang') && session('lang') == 'ru' ? '₽' : '$' }}</div>
                </div>
                <div class="row">
...
<div class="text-page col-md-12"><!-- VK: Лайки ВКонтакте -->
            <ul>
                <li>
                    ...
                    * {{ __("Views for all stories in the last 24 hours") }} <br />
                    * {{ __("60K / Per day") }}<br />
                </li>
                <li>
