...
@section('title', __("SoundCloud Likes") . ' - ' . __('site.site'))
...
@section('content')
    <div class="row">
        <div class="service-content col-md-8">
            ...
            <form name="add_order" method="post" action="{{ route('add_order', ['service' => 'SERVICE_ADD_ORDER_SOUND_CLOUD_LIKES']) }}">
                <input type="hidden" name="type" value="instagram-comments">
                <h1>{{ __("SoundCloud Likes") }}</h1>
                <br>
                <div class="row">
                    <div class="label-name col-md-4">{{ __("Price / 1:") }}</div>
                    <div class="col-md-8 priceOne"><span id="price_base"><strong>{{ socialboosterPrice('SERVICE_ADD_ORDER_SOUND_CLOUD_LIKES_PRICE') }}</strong></span> {{ session()->has('lang') && session('lang') == 'ru' ? '₽' : '$' }}</div>
                </div>
                <div class="row">
...
<div class="text-page col-md-12"><!-- VK: Лайки ВКонтакте -->
            <ul>
               ...
               * {{ __("Likes from real users") }} <br />
                    * {{ __("20 000 Likes / Day") }}<br />
                 ...
                 * {{ __("Min order is") }} 20<br />
                    * {{ __("Max order is") }} 1 000 000 {{ __("If u need more, just contact with our support") }})<br />
