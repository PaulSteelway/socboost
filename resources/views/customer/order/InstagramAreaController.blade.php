...
@section('title', __("Instagram Reach") . ' - ' . __('site.site'))
...
@section('content')
    <div class="row">
        <div class="service-content col-md-8">
            ...
            <form name="add_order" method="post" action="{{ route('add_order', ['service' => 'SERVICE_ADD_ORDER_INSTAGRAM_AREA']) }}">
                <input type="hidden" name="type" value="instagram-comments">
                <h1>{{ __("Instagram Reach") }}</h1>
                <br>
                <div class="row">
                    <div class="label-name col-md-4">{{ __("Price / 1:") }}</div>
                    <div class="col-md-8 priceOne"><span id="price_base"><strong>{{ socialboosterPrice('SERVICE_ADD_ORDER_INSTAGRAM_AREA_PRICE') }}</strong></span> {{ session()->has('lang') && session('lang') == 'ru' ? '₽' : '$' }}</div>
                </div>
                <div class="row">
            ...
            <ul>
                <li>
                 <span class="name">{{ __("Details:") }}</span>
                    ...
                    * {{ __("Reach from real users") }}<br />
                    * {{ __("Your account should be open") }}<br />
                </li>
                <li>
                   <span class="name">{{ __("Limits:") }}</span>
                    <strong>{{ __("Important:") }}</strong>
                    * {{ __("Min order is") }} 100<br />
                    * {{ __("Max order is") }} 1 000 000<br />
