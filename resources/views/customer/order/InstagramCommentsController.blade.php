...
@section('title', __("Instagram comments") . ' - ' . __('site.site'))
...
@section('content')
    <div class="row">
        <div class="service-content col-md-8">
            ...
            <form name="add_order" method="post" action="{{ route('add_order', ['service' => 'SERVICE_ADD_ORDER_INSTAGRAM_COMMENTS']) }}">
                <input type="hidden" name="type" value="instagram-comments">
                <h1>{{ __("Instagram comments") }}</h1>
                <br>
                <div class="row">
                    <div class="label-name col-md-4">{{ __("Price / 1:") }}</div>
                    <div class="col-md-8 priceOne"><span id="price_base"><strong>{{ socialboosterPrice('SERVICE_ADD_ORDER_INSTAGRAM_COMMENTS_PRICE') }}</strong></span> {{ session()->has('lang') && session('lang') == 'ru' ? '₽' : '$' }}</div>
                </div>
                <div class="row">
...
<div class="text-page col-md-12"><!-- VK: Лайки ВКонтакте -->
            <ul>
                <li>
                    ...
                    * {{ __("Comments from real users") }} <br />
                    * {{ __("If you want comments with out text, contact with our support") }}<br />
                </li>
                <li>
                   <span class="name">{{ __("Limits:") }}</span>
                    <strong>{{ __("Important:") }}</strong>
                    * {{ __("Min order is") }} 10 <br />
                    * {{ __("Max order is") }} 1000 ( {{ __("If u need more, just contact with our support") }} )<br />
